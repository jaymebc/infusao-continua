...
function updateCalculations() {
    ...
    const inputs = Array.from(calculatorBody.querySelectorAll('input[type="text"], select')).map(input => ({
        id: input.id
    }));
    
    const params = getParams(false, inputs);
    const { results, dilutionNote } = calculateResults(currentMode, drugData, params, currentPresentationIndex);
    ...
    if (drugData.group_key === 'rocuronio' && currentMode === 'bolus') {
        ...
        if (elInd) {
            elInd.textContent = results[1].value;
        }
        if (elSri) {
            elSri.textContent = results[2].value;
        }
